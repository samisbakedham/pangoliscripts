...
class SafeFactory {
    ...
    async init({ ethAdapter, safeVersion, isL1SafeMasterCopy, contractNetworks }) {
        ...
        const safeProxyFactoryContract = await ethAdapter.getSafeProxyFactoryContract(__classPrivateFieldGet(this, _SafeFactory_safeVersion, "f"), chainId, customContracts === null || customContracts === void 0 ? void 0 : customContracts.safeProxyFactoryAddress);
        if ((await __classPrivateFieldGet(this, _SafeFactory_ethAdapter, "f").getContractCode(safeProxyFactoryContract.getAddress())) === '0x') {
            throw new Error('Safe Proxy Factory contract is not deployed in the current network');
        }
        __classPrivateFieldSet(this, _SafeFactory_safeProxyFactoryContract, safeProxyFactoryContract, "f");
        ...
        if ((await __classPrivateFieldGet(this, _SafeFactory_ethAdapter, "f").getContractCode(gnosisSafeContract.getAddress())) === '0x') {
            throw new Error('Safe Proxy contract is not deployed in the current network');
        }
        __classPrivateFieldSet(this, _SafeFactory_gnosisSafeContract, gnosisSafeContract, "f");
    }
    getEthAdapter() {
    ...
    async deploySafe(safeAccountConfig, safeDeploymentConfig) {
        var _a;
        ...
        const safeContract = await __classPrivateFieldGet(this, _SafeFactory_ethAdapter, "f").getSafeContract({
            safeVersion: __classPrivateFieldGet(this, _SafeFactory_safeVersion, "f"),
            chainId,
            isL1SafeMasterCopy: __classPrivateFieldGet(this, _SafeFactory_isL1SafeMasterCopy, "f"),
            customContractAddress: safeAddress
        });
        if ((await __classPrivateFieldGet(this, _SafeFactory_ethAdapter, "f").getContractCode(safeContract.getAddress())) === '0x') {
            throw new Error('Safe Proxy contract is not deployed in the current network');
        }
